	...
	public static class FairGuardUtils
	{
		public static void Decrypt(Span<byte> bytes)
		{
			unchecked
			{
				try
				{
					Console.WriteLine($"Attempting to decrypt block with FairGuard encryption...");
					var encryptedOffset = 0;
					var encryptedSize = Math.Min(0x500, bytes.Length);
					if (encryptedSize < 0x20)
					{
						Console.WriteLine("block size is less that minimum, skipping...");
						return;
					}
					...
					Console.WriteLine($"encryptedInts[3]: {encryptedInts[3]}, encryptedInts[0]: {encryptedInts[0]}, encryptedSize: {encryptedSize}");
					Console.WriteLine($"XOR result: {encryptedInts[3] ^ encryptedInts[0] ^ 0x7a39d018 ^ encryptedSize}");
					...
					var seedBytes = MemoryMarshal.AsBytes<uint>((Span<uint>)seedInts);
				...
				{
					Console.WriteLine($"Failed to decrypt block with FairGuard encryption: {e.Message}\n{e.StackTrace}");
				}
			}
		}
